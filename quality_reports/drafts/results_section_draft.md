...
Table 2 reports mean VUCA dimension scores in ±2 month windows around five major structural events. The Volatility dimension performs best on face validity grounds, registering above-baseline values at three of five events: the 2019 trade war escalation (+[CHECK: 1.11] SD), the 2020 COVID-19 onset (+[CHECK: 1.44] SD), and [CHECK]. The Complexity dimension shows its strongest face-valid response during the March 2022 Federal Reserve rate hiking cycle (+[CHECK: 1.56] SD), consistent with the interpretation that rapidly rising input costs, land value uncertainty, and credit market tightening jointly increased the dimensionality of farm decision-making environments.
...
- **[CHECK]** Fill in specific SD values from validation output (face validity table): V at trade war = 1.11, COVID = 1.44; C at rate hikes = 1.56
- **[CHECK]** Full-sample means: C_score = 0.794, U_score = −0.396 — confirm these match latest run
